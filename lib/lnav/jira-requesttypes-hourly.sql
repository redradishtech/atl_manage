...
create view buckets AS
	...
	access_log_trunc
	group by 1, 2
	-- Smoosh into groups by date + requesttype.
	-- The HAVING clause lets is say 'ignore /fobar requests if they cumulatively took under 20s to run in an hour
	-- This means:
...
create view _analysis AS select 
	...
	,round(sc_bytes/1024/1024/n,2) AS "mb/req"
	,n
...
:echo Shows high CPU-consuming request types (20s total runtime), grouped by hour, for 24h. See views access_log_trunc, buckets and analysis
